...
KERNEL_BRANCH_NAME=v$(sed -n -e 's/^kernel_version = "\([^"]*\).*/\1/p' data/defaults.toml)
...
git clone https://github.com/accel-ppp/accel-ppp.git -b 1.13.0
./build-accel-ppp.sh
...
git clone https://github.com/maru-sama/rtsp-linux.git -b 475af0a nat-rtsp
./build-nat-rtsp.sh

cd ${ROOTDIR}
mkdir -p build
find packages/linux-kernel/ -type f | grep '\.deb$' | xargs -I {} cp {} build/

cd ${ROOTDIR}/packages
sudo rm -rf debian-repos
# git clone https://github.com/johnlfeeney/debian-repos
git clone https://github.com/TexasInstruments/debian-repos.git -b 09.02.00.009-update
cd debian-repos
sudo ./run.sh ti-linux-firmware
cd ${ROOTDIR}
...
# cp -rf debian-repos/build/trixie/ti-linux-firmware/*64*.deb build/
cp -rf packages/debian-repos/build/bookworm/ti-linux-firmware/*64*.deb build/
